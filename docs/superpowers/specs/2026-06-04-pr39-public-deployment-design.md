...
| ----------------------------------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/web/src/app/api/ops/db-identity/route.ts`       | Create           | Secret-gated diagnostic route. `export const dynamic = "force-dynamic"` (no cache, no static optimization). Returns `{ "currentUser": "<role>" }` only if `BOMY_OPS_DIAGNOSTIC_TOKEN` env is set AND request `x-bomy-ops-token` header matches. Any mismatch or missing env → `new Response(null, { status: 404 })` — body empty, no role/route/env details. Reads runtime DB connection via existing `makeDb()` helper (which reads `DATABASE_URL`), runs `SELECT current_user::text`, returns the role string.      |
| `apps/web/tests/api/ops/db-identity.test.ts`          | Create           | 4 cases: (a) env unset → 404 with empty body; (b) env set + header missing → 404 with empty body; (c) env set + header mismatch → 404 with empty body; (d) env set + header match → 200 with `{ currentUser: "<role>" }` shape (role string asserted to be non-empty; specific value not pinned because CI runs as owner role). Test invocation env: `DATABASE_URL=<bomy_app-conn-str>` (NOT just `DATABASE_APP_URL` — the route reads `DATABASE_URL`), `BOMY_RLS_READY=1`, `BOMY_OPS_DIAGNOSTIC_TOKEN=<test-token>`. |
...
| `apps/web/tests/marketing/membership/actions.test.ts` | Modify or Create | Add a case asserting `joinMembership()` returns the guard error when HITPAY env is unset (without throwing). Extends or creates the existing test file depending on coverage.                                                                                                                                                                                                                                                                                                                                         |
| `apps/web/tests/brands/subscribe/actions.test.ts`     | Modify or Create | Same guard-case as above for `subscribeBrand()` (or whatever the action's exported name is — plan will pin).                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- [ ] `/brands/[any-slug]/subscribe` renders the gated state (no "Subscribe" payment CTA).
