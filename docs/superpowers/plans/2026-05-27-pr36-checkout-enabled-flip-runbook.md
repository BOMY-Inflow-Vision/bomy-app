...
| `packages/db/scripts/ops/platform-config-flip.ts`                    | Thin CLI wrapper. Reads `process.argv`, builds DB via `makeDb()`, calls core, formats stdout/stderr, calls `process.exit(0                                                                                               | 1   | 2)`. |
